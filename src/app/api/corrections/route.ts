...
export async function GET(request: Request) {
  try {
    ...
    const wsId = url.searchParams.get("workspace_id") ?? request.headers.get("x-crisp-workspace-id") ?? (await defaultWorkspaceId());
    await requireContext(request, wsId); // FR-34: Viewers can view corrections
    const rows = await listCorrections(wsId, documentId || undefined);
...
export async function POST(request: Request) {
  try {
    ...
    const wsId = log.workspace_id || (await defaultWorkspaceId());
    ...
    if (log.document_ids) {
      ...
      const owned = new Set((await (await import("@/lib/db")).listDocuments(wsId)).map((d) => d.id));

...
function migrate(db: Database.Database) {
  ...
  db.exec(`
    ...
    -- Persistent, resumable chat sessions (Phase 1 of persistence/enterprise plan).
...
export function rawQueryOne<T>(sql: string, params: unknown[] = []): T | undefined {
  const stmt = getDb().prepare(sql);
...
export function deleteWorkspaceCascade(workspaceId: string): void {
  ...
  const tx = db.transaction(() => {
    ...
    // Chat sessions/messages have a workspace_id but messages don't; delegate to the cascade helper.
...
/* ------------------------- Chat sessions & messages (persistent, Phase 1) ------------------------- */
...
export function insertChatSession(input: NewChatSessionInput): ChatSessionRow {
  const id = input.id ?? "chat_" + randomUUID();
  getDb()
    .prepare(
      `INSERT INTO chat_sessions (id, user_id, workspace_id, title, document_ids, status, created_at, updated_at)
       VALUES (@id, @user_id, @workspace_id, @title, @document_ids, @status, strftime('%Y-%m-%dT%H:%M:%fZ','now'), strftime('%Y-%m-%dT%H:%M:%fZ','now'))`
    )
    .run({
      id,
...
export function getChatSession(id: string): ChatSessionRow | undefined {
  return getDb().prepare("SELECT * FROM chat_sessions WHERE id = ?").get(id) as ChatSessionRow | undefined;
}

export function listChatSessions(userId: string, workspaceId: string): ChatSessionRow[] {
  return getDb()
    .prepare(
    ...
    .all(userId, workspaceId) as ChatSessionRow[];
}

export function touchChatSession(id: string, documentIds: string[]): void {
  getDb()
    .prepare(
    ...
    .run(JSON.stringify(documentIds), id);
}

export function renameChatSession(id: string, title: string): void {
  getDb()
    .prepare("UPDATE chat_sessions SET title = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = ?")
    .run(title, id);
}
...
export interface NewChatMessageInput {
  ...
  content: unknown; // JSON blob; shape depends on role
  query_log_id?: string | null;
}

export function insertChatMessage(input: NewChatMessageInput): ChatMessageRow {
  const id = input.id ?? "msg_" + randomUUID();
  getDb()
    .prepare(
      `INSERT INTO chat_messages (id, session_id, role, content, query_log_id)
       VALUES (@id, @session_id, @role, @content, @query_log_id)`
    )
    .run({
      id,
      ...
      query_log_id: input.query_log_id ?? null,
    });
  return getChatMessage(id)!;
}

export function getChatMessage(id: string): ChatMessageRow | undefined {
...
/** Schedules a session for archival; does not delete content. */
export function archiveChatSession(id: string): void {
  getDb()
    .prepare("UPDATE chat_sessions SET status = 'archived', updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = ?")
    .run(id);
}

/** Hard-deletes a session and all its messages (used by data-deletion flows). */
...
/** Hard-deletes every session and message for a workspace (teardown cascade). */
export function deleteChatSessionsForWorkspace(workspaceId: string): void {
  const db = getDb();
  const tx = db.transaction(() => {
    const ids = (db.prepare("SELECT id FROM chat_sessions WHERE workspace_id = ?").all(workspaceId) as Array<{ id: string }>).map((r) => r.id);
    if (ids.length) {
      const ph = ids.map(() => "?").join(",");
      db.prepare(`DELETE FROM chat_messages WHERE session_id IN (${ph})`).run(...ids);
      db.prepare(`DELETE FROM chat_sessions WHERE id IN (${ph})`).run(...ids);
    }
  });
  tx();
}
